...
TEST_F(ClientCacheManagerUnitTest, ChunkHandleMappingTest) {
  ...
  for (int i = 0; i < num_of_chunk; i++) {
    auto set_status(manager->SetChunkHandle(filename, i, std::to_string(i)));
  ...
  for (int i = 0; i < num_of_chunk; i++) {
    ...
    EXPECT_EQ(get_status.value(), std::to_string(i));
  }
  ...
  EXPECT_TRUE(IsInvalidArgument(set_status));
  ...
  EXPECT_TRUE(IsNotFound(get_status.status()));
...
TEST_F(ClientCacheManagerUnitTest, ChunkVersionTest) {
  ...
  EXPECT_TRUE(get_status.ok());
  EXPECT_EQ(get_status.value(), 0);

  manager->SetChunkVersion(chunk_handle, 100);
  EXPECT_EQ(manager->GetChunkVersion(chunk_handle).value(), 100);
...
TEST_F(ClientCacheManagerUnitTest, ChunkServerLocationTest) {
  ...
  for (int i = 0; i < num_of_chunk; i++) {
    ...
    auto set_chunk_status(manager->SetChunkHandle(filename, i, chunk_handle));
  ...
  for (int i = 0; i < num_of_chunk; i++) {
    std::string chunk_handle(std::to_string(i));
    auto get_chunk_location_or(manager->GetChunkServerLocation(chunk_handle));
    EXPECT_TRUE(get_chunk_location_or.ok());

    CacheManager::ChunkServerLocationEntry cache_entry(
        get_chunk_location_or.value());
  ...
  for (int i = 0; i < num_of_chunk; i++) {
    ...
    EXPECT_TRUE(IsDeadlineExceeded(get_chunk_location_or.status()));
  }
  ...
  EXPECT_TRUE(get_chunk_location_or.ok());
  auto cache_entry = get_chunk_location_or.value();
