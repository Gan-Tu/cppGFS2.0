...
TEST_F(MetadataManagerUnitTest, UpdateChunkMetadataInParallelTest) {
    ...
    threads.push_back(std::thread([&, i]() {
      ...
      for (int chunk_id = 0; chunk_id < num_of_chunk_per_file; chunk_id++) {
        ...
        InitializeChunkMetadata(chunk_data, chunk_handle, 0,
                                ...
                                // Create chunk server replicas, with the first
                                // port equal to 5000 + chunk_id, and second
                                // port 5001 + chunk_id and the third 5002
        ...
        metadata_manager_->SetPrimaryLeaseMetadata(
            chunk_handle, new_primary_location, 100000);
      }
    }));
  }
  ...
  for (int i = 0; i < num_of_threads; i++) {
    for (int chunk_id = 0; chunk_id < num_of_chunk_per_file; chunk_id++) {
      ...
      auto result_or = metadata_manager_->GetPrimaryLeaseMetadata(chunk_handle);
      EXPECT_TRUE(result_or.second);
      EXPECT_EQ(result_or.first.first.server_port(), 5002);
      EXPECT_EQ(result_or.first.second, 100000);
