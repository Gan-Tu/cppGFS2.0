 ...
 * 4) delete a file, this involves the deletion of all chunk handles assigned to this
 *    file.
...
class MetadataManager {
   public:
      ...
      /* Create the file metadata (and a lock associated with this file) for a given
       * file path. This function returns false if the file path already exists or if
       * any of the intermediate parent directory does not exist. */
      bool CreateFileMetadata(const std::string& pathname);

      /* Check if metadata file a file exists */
      bool ExistFileMetadata(const std::string& pathname);
      
      /* Access the file metadata for a given file path. The caller of this function
       * needs to ensure the lock for this file is properly used */
      std::shared_ptr<FileMetadata> GetFileMetadata(const std::string& pathname);

      /* Create a file chunk for a given pathname and a chunk index.  */
      std::string CreateChunkHandle(const std::string& pathname, uint32_t chunk_index);
